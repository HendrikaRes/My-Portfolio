...
export default function Skills({ lang }) {
  ...
  const skills = [
    ...
    { name: "Python", icon: <SiPython className="w-8 h-8 md:w-10 md:h-10 text-[#3776AB]" /> },
    { name: "Javascript", icon: <SiJavascript className="w-8 h-8 md:w-10 md:h-10 text-[#F7DF1E]" /> },
    { name: "Java", icon: <FaJava className="w-8 h-8 md:w-10 md:h-10 text-[#007396]" /> },
    { name: "PHP", icon: <SiPhp className="w-8 h-8 md:w-10 md:h-10 text-[#777BB4]" /> },
      ...
      <div className="max-w-5xl mx-auto text-center">
        ...
        <div className="mb-14 flex flex-col items-center">
          <h2 className="text-4xl md:text-5xl font-black text-white tracking-tight mb-4">
            {lang === 'ID' ? (
              <>
                Keahlian <span className="text-transparent bg-clip-text bg-linear-to-r from-blue-400 to-emerald-400">Saya</span>
              </>
            ) : (
              <>
          ...
          <p className="text-slate-400 max-w-2xl text-base md:text-lg leading-relaxed">
            {lang === 'ID' 
              ...
              : 'Technologies and tools I use daily to develop reliable software systems.'}
          </p>
        </div>
